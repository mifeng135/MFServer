...
typedef struct mp_buf {
    unsigned char *b;
    size_t len, free;
    unsigned char *heap_b;      /* Pointer to heap-allocated buffer */
    unsigned char stack_buf[128]; /* Stack-allocated small buffer for optimization */
    int is_stack;               /* Flag: 1 if using stack buffer, 0 if using heap */
} mp_buf;
...
mp_buf *mp_buf_new(lua_State *L) {
    ...
    /* Initialize with stack buffer for small data optimization */
    buf->b = buf->stack_buf;
    buf->heap_b = NULL;
    buf->len = 0;
    buf->free = 128;
...
void mp_buf_append(lua_State *L, mp_buf *buf, const unsigned char *s, size_t len) {
    if (buf->free < len) {
        ...
        if (buf->is_stack) {
            ...
            buf->heap_b = (unsigned char*)mp_realloc(L, NULL, 0, newsize);
            
            /* Copy existing data from stack to heap */
            if (buf->len > 0) {
                memcpy(buf->heap_b, buf->stack_buf, buf->len);
            }
            ...
            /* Already on heap, normal reallocation */
            buf->heap_b = (unsigned char*)mp_realloc(L, buf->heap_b, buf->len + buf->free, newsize);
            buf->b = buf->heap_b;
            buf->free = newsize - buf->len;
        }
    }
...
void mp_buf_free(lua_State *L, mp_buf *buf) {
    /* Only free heap-allocated buffer, stack buffer is auto-freed */
    if (buf->heap_b != NULL) {
        mp_realloc(L, buf->heap_b, buf->len + buf->free, 0); /* realloc to 0 = free */
    }
...
void mp_encode_int(lua_State *L, mp_buf *buf, int64_t n) {
    ...
    if (n >= 0) {
        if (n <= 127) {
            b[0] = (unsigned char)n;    /* positive fixnum */
            enclen = 1;
        } else if (n <= 0xff) {
            b[0] = 0xcc;        /* uint 8 */
            b[1] = (unsigned char)n;
        ...
        if (n >= -32) {
            b[0] = (unsigned char)((signed char)n);   /* negative fixnum */
            enclen = 1;
        } else if (n >= -128) {
            b[0] = 0xd0;        /* int 8 */
            b[1] = (unsigned char)((int8_t)n);
            enclen = 2;
        } else if (n >= -32768) {
            int16_t val = (int16_t)n;
            b[0] = 0xd1;        /* int 16 */
            b[1] = (val >> 8) & 0xff;
            b[2] = val & 0xff;
            enclen = 3;
...
static inline void mp_encode_array(lua_State *L, mp_buf *buf, int64_t n) {
    unsigned char b[5];
    int enclen;
...
void mp_encode_lua_table_as_array(lua_State *L, mp_buf *buf, int level) {
    ...
    for (j = 1; j <= len; j++) {
        lua_rawgeti(L, -1, (int)j);  /* Optimized: use rawgeti instead of gettable */
...
/* Convert a lua table into a message pack key-value map.
 * Optimized: single-pass encoding with header backfill */
void mp_encode_lua_table_as_map(lua_State *L, mp_buf *buf, int level) {
    size_t len = 0;
    size_t start_pos = buf->len;  /* Remember where map header starts */
    
    /* Reserve maximum header size (5 bytes for map32) */
    unsigned char placeholder[5] = {0, 0, 0, 0, 0};
    mp_buf_append(L, buf, placeholder, 5);
    size_t data_start = buf->len;
    
    /* Single pass: count and encode at the same time */
    ...
    /* Calculate actual header length needed */
    int actual_header_len;
    unsigned char header[5];
    
    if (len <= 15) {
        header[0] = 0x80 | (len & 0xf);    /* fix map */
        actual_header_len = 1;
    } else if (len <= 65535) {
        header[0] = 0xde;                  /* map 16 */
        header[1] = (len & 0xff00) >> 8;
        header[2] = len & 0xff;
        actual_header_len = 3;
    } else {
        header[0] = 0xdf;                  /* map 32 */
        header[1] = (len & 0xff000000) >> 24;
        header[2] = (len & 0xff0000) >> 16;
        header[3] = (len & 0xff00) >> 8;
        header[4] = len & 0xff;
        actual_header_len = 5;
    }
    
    /* If we reserved more space than needed, shift data left */
    if (actual_header_len < 5) {
        size_t data_size = buf->len - data_start;
        memmove(buf->b + start_pos + actual_header_len, 
                buf->b + data_start, 
                data_size);
        buf->len = start_pos + actual_header_len + data_size;
    }
    
    /* Write the correct header */
    memcpy(buf->b + start_pos, header, actual_header_len);
}

/* Returns true if the Lua table on top of the stack is exclusively composed
 * of keys from numerical keys from 1 up to N, with N being the total number
 * of elements, without any hole in the middle.
 * Optimized: early exit and faster checks */
int table_is_an_array(lua_State *L) {
    ...
    /* Fast path: if length is 0, it's not an array */
    if (len == 0) return 0;
    
    /* Fast path: check if all keys from 1 to len exist */
    /* This is much faster than iterating through all keys */
    int count = 0;
    ...
    /* Count all keys and check if any is not a valid array index */
    lua_pushnil(L);
    while(lua_next(L,-2)) {
        /* Stack: ... key value */
        lua_pop(L,1); /* Stack: ... key */
        
        /* Check if key is a valid positive integer */
#if LUA_VERSION_NUM < 503
        if ((LUA_TNUMBER != lua_type(L,-1)) || (n = lua_tonumber(L, -1)) <= 0 ||
            !IS_INT_EQUIVALENT(n) || n > len)
#else
        if (!lua_isinteger(L,-1) || (n = lua_tointeger(L, -1)) <= 0 || n > (long long)len)
#endif
        {
            lua_settop(L, stacktop);
            return 0;  /* Early exit: found non-array key */
        }
        count++;
        
        /* Early exit: too many keys */
        if (count > (int)len) {
            lua_settop(L, stacktop);
            return 0;
        }
    ...
    /* It's an array if count matches the length */
    lua_settop(L, stacktop);
...
int registerMsgFunc(lua_State *L) {
    luaL_Reg l[] = {
        {"pack", mp_pack},
        {"unpack", mp_unpack},
...
LUALIB_API int luaopen_cmsgpack_safe(lua_State *L) {
    luaL_requiref(L, "MF.msgPack", registerMsgFunc, 1);
    lua_pop(L, 1);
    return 1;
}
